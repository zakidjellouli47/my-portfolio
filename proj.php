...
$(document).ready(function(){
 ...
 checkbox.click(function(){
  ...
  var selectedString = selected.join(",");
        $.post("ajaxdeleteselected4.php", {selected: selected },
        function(data){
...
if(isset($_POST["cmdaddnew"])){
 $sql = "INSERT INTO projects (project,level_of_priority)
 VALUES ('".$_POST["project"]."','".$_POST["level_of_priority"]."')";
...
if(isset($_POST["cmdelete"])){
   $sql = "DELETE FROM projects WHERE id='$deleteemployee'";
   if ($conn->query($sql) === TRUE) {
  ...
  echo '<meta http-equiv="refresh" content="1;url=proj.php" />';    exit;
...
if ($editemployee==''){
 ...
 $results = mysqli_query($conn,"SELECT * FROM  projects Where  id=$editemployee");
 while($row = mysqli_fetch_array($results))
 {
  $editid = $row["id"];
  $edit_fullname = $row["project"];
$edit_nom = $row["level_of_priority"];
...
if(isset($_POST["cmdedit"])){
   $editname = $_POST["project"];

  
   $editannee  = $_POST["level_of_priority"];
   ...
   $sql = "UPDATE projects SET project='$editname' ,level_of_priority='$editannee'  WHERE id='$editemployee'";
   if (mysqli_query($conn, $sql)) {
     echo "<script type= 'text/javascript'>alert('record updated successfully');
  window.location.replace('proj.php');</script>";  
   } else {
...
$last_btn = true;
$start = $page * $per_page;
$query_pag_data = "SELECT * from  projects ORDER BY level_of_priority  DESC LIMIT $start, $per_page";
...
<div class="container"><div class = "result"></div>
        ...
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
      <h2>Lists<b> of Projects</b></h2>
     </div>
     <div class="col-sm-6">
    
      <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i class="material-icons"></i> <span>Add a project </span></a>
...
<td><?php echo $Type; ?></td>
                        ...
                        <td>
                            <a href="?editemployee=<?php echo $row['id']; ?>" class="edit" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Modify"></i></a>
   ...
   <div class="clearfix">
   <a style="text-align: center; margin-left: 200px;"  href="Home.html" class="page-link">Home_Page</a>
   

<?php
$msg = "";
$query_pag_num = mysqli_query($conn,"SELECT COUNT(*) AS mycount FROM projects" ) or die(mysqli_error($this->dblink));
...
if ($previous_btn && $cur_page > 1) {
    $pre = $cur_page - 1;
    $msg .= "<li p='$pre' class='page-item active'><a href='?page=$pre' class='page-link'>Prev</a></li>";
} else if ($previous_btn) {
    $msg .= "<li class='page-item' disabled><a href='?page=1'>Prev</a></li>";
}
...
$total_string = "<div class=\"hint-text\">Display <b>" . $cur_page . "</b> from <b>$no_of_paginations</b> entries</div>";
$msg = $msg . "</ul>";  
echo $total_string;
echo $msg;
...
<div class="form-group">
       <label>Level_of_Priority</label>
       <input type="text" name="level_of_priority" class="form-control" required>
      </div>
 ...
 <div id="editEmployeeModal" class="modal fade">
  <div class="modal-dialog">
   <div class="modal-content">
    <form name="frmedit" action="" method="post">
     <div class="modal-header">      
      <h4 class="modal-title">Modify a project </h4>
      <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
     </div>
     <div class="modal-body">
      ...
      <div class="form-group">
       ...
       <input type="text" name="project" value="<?php echo $edit_fullname; ?>" class="form-control" required>
      </div>
  ...
  <div class="form-group">
       <label>Level_of_Priority</label>
       <input type="text" name="level_of_priority" value="<?php echo $edit_nom; ?>" class="form-control" required>
      </div>
  ...
  <div class="modal-dialog">
   <div class="modal-content">
    <form name="frmdelete" action="" method="post">
     <div class="modal-header">      
      <h4 class="modal-title">Delete the selected Project</h4>
      <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
     </div>
     <div class="modal-body">     
      <p>Are you sure you want to delete the records?</p>
      <p class="text-warning"><small>This action cannot be canceled.</small></p>
     </div>
     <div class="modal-footer">
      <input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel">
